...
class Heptathlon:
    ...
    def total(self):
        """
        Calcul le total des épreuves du heptathlon

        Returns:
            int: total du heptathlon
        """
        total = 0
    ...
    def details(self) -> None:
        ...
        for index in range(len(self.perfs)):
            resultats.add_row(
                [
                    self.perfs[index].nom,
                    (str(self.perfs[index].perf) + "" + self.perfs[index].unite),
                    self.perfs[index].score,
                ]
            )
        print(resultats)
