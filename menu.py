...
def ajouter_perfomances(compet_chargee):
    ...
    for epreuve in range(len(nom_epreuves)):
        performances.append(
            float(input("Perfomance -> " + nom_epreuves[epreuve] + " : "))
        )
    system("clear")
    ...
    add_confirmation = TerminalMenu(["OUI", "NON"])
...
def new_competition():
    ...
    liste_type_competition = ["Décathlon", "Heptathlon"]
...
def details(compet_chargee):
    ...
    if field == 0:
        ...
        if len(compet_chargee.combined_event) > 0:
            ...
            for ce in range(len(compet_chargee.combined_event)):
                perfs[compet_chargee.combined_event[ce].athlete.nom] = (
                    compet_chargee.combined_event[ce].perfs[ep_index].perf
                )
    ...
    # Par athlète
        ...
        liste_nom_athlete = []
        for ce in range(len(compet_chargee.combined_event)):
            liste_nom_athlete.append(
                compet_chargee.combined_event[ce].athlete.prenom
                + " "
                + compet_chargee.combined_event[ce].athlete.nom
            )
        ...
        print(
            "Athlete : "
            + compet_chargee.combined_event[athlete].athlete.prenom
            + " "
            + compet_chargee.combined_event[athlete].athlete.nom
        )
        ...
        for epreuve in range(len(compet_chargee.combined_event[athlete].perfs)):
            resultats_athlete.add_row(
                [
                    ...
                    str(compet_chargee.combined_event[athlete].perfs[epreuve].perf)
                    + " "
                    + compet_chargee.combined_event[athlete].perfs[epreuve].unite,
